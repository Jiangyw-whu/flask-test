...
def get_tencent_data():#编写代码获取腾讯新闻的疫情数据
    """
    ...
    r=requests.get(url,headers)  # 执行返回json格式的数据
    #使用requests请求
    res=json.loads(r.text) #将son格式的数据，转换为字典
    data_all=json.loads(res["data"])#再用json模块加载data里的数据到新的变量data_all（即str加载成字典格式）
